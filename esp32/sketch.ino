...
#define TOPIC_TELEMETRIA "sentineleye/posto01/telemetria"
#define TOPIC_ALERTA "sentineleye/posto01/alerta"
#define TOPIC_STATUS "sentineleye/posto01/status"
...
void connectMQTT() {
  while (!client.connected()) {
    String clientId = "ESP32-SentinelPost-" + String(random(1000, 9999));

    if (client.connect(clientId.c_str())) {
      Serial.println("MQTT conectado");
      client.publish(TOPIC_STATUS, "SentinelPost online");
    } else {
...
void loop() {
  ...
  int gas = map(leituraGas, 0, 4095, 0, 100);
  ...
  if (movimento == HIGH && luminosidade < 30) {
    risco = 1;
  }

  if (gas > 60 || temperatura > 40) {
    risco = 2;
  }
